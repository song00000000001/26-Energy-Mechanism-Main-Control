...
void xSDA_IN(IIC_PIN_Typedef *iic_pin)              		    
{
  ...
  uint32_t position = iic_pin->IIC_SDA_PIN_NUM;
  __IO uint32_t *configregister; /* Store the address of CRL or CRH register based on pin number */
  uint32_t registeroffset;       /* offset used during computation of CNF and MODE bits placement inside CRL or CRH register */

  configregister = (position < 8) ? &iic_pin->IIC_GPIO_PORT->CRH : &iic_pin->IIC_GPIO_PORT->CRH;
  registeroffset = (position < 8) ? (position << 2u) : ((position - 8u) << 2u);

  MODIFY_REG((*configregister), ((GPIO_CRL_MODE0 | GPIO_CRL_CNF0) << registeroffset), ((GPIO_CR_MODE_INPUT + GPIO_CR_CNF_INPUT_PU_PD) << registeroffset));
#elif defined(USE_STM32H7)
  uint32_t temp;
  uint32_t position = iic_pin->IIC_SDA_PIN_NUM;
  GPIO_TypeDef *GPIOx = iic_pin->IIC_GPIO_PORT;
  temp = GPIOx->MODER;
  temp &= ~(GPIO_MODER_MODE0 << (position * 2U));
  temp |= ((GPIO_MODE_INPUT & GPIO_MODE) << (position * 2U));
  GPIOx->MODER = temp;
#else
  iic_pin->IIC_GPIO_PORT->MODER &= ~(3 << (iic_pin->IIC_SDA_PIN_NUM * 2)); // 清零
  iic_pin->IIC_GPIO_PORT->MODER |= (0 << (iic_pin->IIC_SDA_PIN_NUM * 2));  // 输入
#endif
}
...
void xSDA_OUT(IIC_PIN_Typedef *iic_pin)             		    
{
  ...
  MODIFY_REG((*configregister), ((GPIO_CRL_MODE0 | GPIO_CRL_CNF0) << registeroffset), ((GPIO_SPEED_FREQ_HIGH + GPIO_CR_CNF_GP_OUTPUT_OD) << registeroffset));
#elif defined(USE_STM32H7)
  uint32_t temp;
  uint32_t position = iic_pin->IIC_SDA_PIN_NUM;
  temp = iic_pin->IIC_GPIO_PORT->MODER;
  temp &= ~(GPIO_MODER_MODE0 << (position * 2U));
  temp |= ((GPIO_MODE_OUTPUT_OD & GPIO_MODE) << (position * 2U));
  iic_pin->IIC_GPIO_PORT->MODER = temp;
#else
  iic_pin->IIC_GPIO_PORT->MODER &= ~(3 << (iic_pin->IIC_SDA_PIN_NUM * 2)); // 清零
  iic_pin->IIC_GPIO_PORT->MODER |= (1 << (iic_pin->IIC_SDA_PIN_NUM * 2));  // 通用输出模式
#endif
}
