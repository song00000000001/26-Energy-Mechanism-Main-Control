...
template <typename Type = float, size_t M = 1, size_t N = 1>
class Matrix
{
public:
  Type _data[M][N]{};
    ...
    for (size_t i = 0; i < M; i++)
    {
      for (size_t k = 0; k < P; k++)
      {
        for (size_t j = 0; j < N; j++)
        {
          ...
          res._data[i][k] += self._data[i][j] * other._data[j][k];
        }
      }
    }
  ...
  Matrix<Type, M, N> operator*(Type scalar) const
  {
    ...
    for (size_t i = 0; i < M; i++)
    {
      for (size_t j = 0; j < N; j++)
      {
        res._data[i][j] = self._data[i][j] * scalar;
      }
    }
  ...
  Matrix<Type, M, N> abs() const
  {
    ...
    for (size_t i = 0; i < M; i++)
    {
      for (size_t j = 0; j < N; j++)
      {
        r._data[i][j] = Type(fabs((*this)._data[i][j]));
      }
    }
    return r;
  }
  ...
  bool isAllNan() const
  {
    ...
    for (size_t i = 0; i < M; i++)
    {
      for (size_t j = 0; j < N; j++)
      {
        result = result && isnan(self._data[i][j]);
      }
    }
    return result;
  }
             ...
             const Matrix<Type, M, N>& y, const Type eps = Type(1e-4f))
{
  for (size_t i = 0; i < M; i++)
  {
    for (size_t j = 0; j < N; j++)
    {
      if (!isEqualF(x._data[i][j], y._data[i][j], eps))
      {
        return false;
      }
    }
...
Matrix<Type, M, N> min(const Matrix<Type, M, N>& x, const Type scalar_upper_bound)
{
  ...
  for (size_t i = 0; i < M; i++)
  {
    for (size_t j = 0; j < N; j++)
    {
      m._data[i][j] = typeFunction::min(x._data[i][j], scalar_upper_bound);
    }
  }
  return m;
}
...
Matrix<Type, M, N> min(const Matrix<Type, M, N>& x1, const Matrix<Type, M, N>& x2)
{
  ...
  for (size_t i = 0; i < M; i++)
  {
    for (size_t j = 0; j < N; j++)
    {
      m._data[i][j] = typeFunction::min(x1._data[i][j], x2._data[i][j]);
    }
  }
  return m;
}
...
Matrix<Type, M, N> max(const Matrix<Type, M, N>& x, const Type scalar_lower_bound)
{
  ...
  for (size_t i = 0; i < M; i++)
  {
    for (size_t j = 0; j < N; j++)
    {
      m._data[i][j] = typeFunction::max(x._data[i][j], scalar_lower_bound);
    }
  }
  return m;
}
...
Matrix<Type, M, N> max(const Matrix<Type, M, N>& x1, const Matrix<Type, M, N>& x2)
{
  ...
  for (size_t i = 0; i < M; i++)
  {
    for (size_t j = 0; j < N; j++)
    {
      m._data[i][j] = typeFunction::max(x1._data[i][j], x2._data[i][j]);
    }
  }
  return m;
}
                             ...
                             const Type scalar_upper_bound)
{
  ...
  else
  {
    for (size_t i = 0; i < M; i++)
    {
      for (size_t j = 0; j < N; j++)
      {
        m._data[i][j] = typeFunction::constrain(x._data[i][j], scalar_lower_bound, scalar_upper_bound);
      }
    }
  }
  return m;
                             ...
                             const Matrix<Type, M, N>& x_upper_bound)
{
  ...
  for (size_t i = 0; i < M; i++)
  {
    for (size_t j = 0; j < N; j++)
    {
      m._data[i][j] = typeFunction::constrain(x._data[i][j], x_lower_bound._data[i][j], x_upper_bound._data[i][j]);
    }
  }
  return m;
}
